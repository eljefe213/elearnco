...
import { IBlobReturnType, IUpload } from "./types/types";
import { FileType } from "schemas";
...
export const useUploadFile = ({
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      props.fileType &&
      !props.fileType.includes(filelist[0].type as FileType)
    )
      return handleFileTypeValidations({
        fileType: props.fileType as unknown as FileType,
        event: event,
        handleError: props.handleError,
      });
  ...
  const handleOnDrop = async (event: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      const resolvedFiles = result
        ...
        .map((result: Record<string, any>) => result.value as string);
